...
import data.datasets.movielens.movielens_parsing as movielens_parsing
...
class Movielens(tfds.core.GeneratorBasedBuilder):
    """DatasetBuilder for movielens dataset."""
    ...
    BUILDER_CONFIGS = [
        MovieLensConfig(
            ...
            parsing_fn=movielens_parsing.parse_ratings_data,
        ),
        MovieLensConfig(
            name='10m-movies',
            description=textwrap.dedent("""\
            ...
            table_option='movies',
            download_url='http://files.grouplens.org/datasets/movielens/ml-10m.zip',
            parsing_fn=movielens_parsing.parse_movies_data,
        )
    ]
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        if os.path.exists(os.path.join(extracted_path, 'ml-10M100K')):
            os.rename(os.path.join(extracted_path, 'ml-10M100K'), os.path.join(extracted_path, 'ml-10m'))
        dir_path = os.path.join(
            extracted_path,
            f'ml-{self.builder_config.format_version}',
        )
        return [
