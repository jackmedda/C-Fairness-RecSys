...
records = read_delim("data/lastfm-dataset-360K/usersha1-artmbid-artname-plays.tsv",
                     delim="\t", quote="",
                     col_names=c("user", "artid", "plays"), col_types="cc_i",
                     progress=TRUE)
...
message("joining gender info in order to sample")
user_gender = play_counts %>%
  select(user) %>%
  distinct() %>%
  inner_join(users %>% select(user=id,gender))

lfm_distinct_M = user_gender %>%
  filter(gender=='m')
lfm_distinct_F = user_gender %>%
  filter(gender=='f')
...
lfm_sample_F = lfm_distinct_F %>%
  sample_n(size=size,replace=FALSE)
message(sprintf("sampled %d female and %d male users",
                nrow(lfm_sample_F), nrow(lfm_sample_M)))

users_sample = bind_rows(lfm_sample_M %>% select(id=user),
                         lfm_sample_F %>% select(id=user)) %>%
    inner_join(users %>% select(id, key, gender, age))
write_csv(users_sample, "build/lfm-users-sample.csv")

message("joining sample users with ratings")
play_counts_sample = users_sample %>%
    select(user=id) %>%
    inner_join(play_counts) %>%
    select(user, item, count)

message("writing sampled data")
write_csv(play_counts_sample, "build/lfm-ratings-sample.csv", na="")
...
message("partitioning sample data")
user.counts = play_counts_sample %>%
  group_by(user) %>%
  summarize(nratings = n())
item.counts = play_counts_sample %>%
  group_by(item) %>%
  summarize(nratings = n())
...
test_user_ratings = play_counts_sample %>%
  inner_join(test_users) %>%
  group_by(user) %>%
  mutate(rank = sample(n())) %>%
  filter(rank <= 5)
...
test_item_ratings = play_counts_sample %>%
  inner_join(test_items) %>%
  group_by(item) %>%
  mutate(rank = sample(n())) %>%
  filter(rank <= 5)

dir.create("build/lastfm-splits-sample", showWarnings=FALSE)
for (i in 1:5) {
  message(sprintf("writing sample partition %d", i))
  testr = test_user_ratings %>% filter(partition == i)
  trainr = play_counts_sample %>% left_join(select(testr, user, item, partition)) %>% filter(is.na(partition)) %>% select(user, item, count)
  write_csv(testr %>% select(user, item, count),
            sprintf("build/lastfm-splits-sample/u.part%d.test.csv", i), na="")
  write_csv(trainr, sprintf("build/lastfm-splits-sample/u.part%d.train.csv", i), na="")
  testr = test_item_ratings %>% filter(partition == i)
  trainr = play_counts_sample %>% left_join(select(testr, user, item, partition)) %>% filter(is.na(partition)) %>% select(user, item, count)
  write_csv(testr %>% select(user, item, count),
            sprintf("build/lastfm-splits-sample/i.part%d.test.csv", i), na="")
  write_csv(trainr, sprintf("build/lastfm-splits-sample/i.part%d.train.csv", i), na="")
}
