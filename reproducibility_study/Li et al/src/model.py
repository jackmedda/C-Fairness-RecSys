...
class UGF(object):
    ...
    def train(self):
        ...
        all_df.to_csv(os.path.join(
            ...
            os.pardir,
            'out_results',
            f"{model_name}_{run_id}_{group_name_title}_out.csv"
        ), index=False)
        exit()
        #####################################
...
if __name__ == '__main__':
    ...
    model_name = 'filtered(20)_lastfm_1K_BiasedMF'                      # model name (which model does this ranking file generated by)
    group_name_title = '___per_user_timestamp_split[80%, 20%]'           # grouping method name for distinguish different experiment results
    ...
    run_id = '41d1bbe242'
    rank_file = [f for f in os.listdir(dataset_folder) if model_name in f and 'rank.csv' in f and run_id in f][0]   # original input ranking csv file name
    group_name_title = rank_file.split('_')[1] + group_name_title

    group_1_file = [f for f in os.listdir(dataset_folder) if model_name in f and group_1 in f and run_id in f][0]      # advantaged group testing file name
    group_2_file = [f for f in os.listdir(dataset_folder) if model_name in f and group_2 in f and run_id in f][0]    # disadvantaged group testing file name
    ############################################################################
