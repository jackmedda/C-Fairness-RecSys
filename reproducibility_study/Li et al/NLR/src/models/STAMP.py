...
class STAMP(GRU4Rec):
    ...
    def predict(self, feed_dict):
        ...
        pos_his_vectors = self.iid_embeddings(history.abs().long() * valid_his) * \
                          valid_his.unsqueeze(dim=-1).float()  # B * H * V
        if self.neg_emb == 1:
            neg_his_vectors = self.iid_embeddings_neg(history.abs().long() * valid_his) * valid_his.unsqueeze(dim=-1).float()
