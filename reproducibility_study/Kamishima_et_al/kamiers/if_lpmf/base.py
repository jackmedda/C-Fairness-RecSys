...
from ..base import BaseIndependentExplicitItemFinderFromSingleBinarySensitive
...
class BaseIndependentLogisticPMF(
        with_metaclass(
            ABCMeta,
            BaseIndependentExplicitItemFinderFromSingleBinarySensitive)):
    """
    Base class for a logistic probabilistic matrix factorization model
    ...
    method_name = 'logistic_pmf_base'

    def __init__(self, C=1.0, k=1, eta=1.0, random_state=None):
        super(BaseIndependentLogisticPMF, self).__init__(
    ...
    def _init_coef(self, orig_data, sev, ssc, n_objects, **optimizer_kwargs):
        ...
        rec = LogisticPMF(
            C=self.C, k=k, random_state=self._rng, **optimizer_kwargs)
    ...
    def remove_data(self):
        ...
        """
        super(BaseIndependentLogisticPMF, self).remove_data()
    ...
    def raw_predict(self, ev, sen):
        ...
        return sigmoid([self.mu_[s][0] +
                        self.bu_[s][ev[i, 0]] + self.bi_[s][ev[i, 1]] +
                        np.dot(
                            self.p_[s][ev[i, 0], :], self.q_[s][ev[i, 1], :])
                        for i, s in enumerate(sen)])


class BaseIndependentLogisticPMFWithOptimizer(
        with_metaclass(ABCMeta, BaseIndependentLogisticPMF)):
    """
    BaseIndependentLogisticPMF: fit method using a optimizer
    ...
    def __init__(
        ...
        super(BaseIndependentLogisticPMFWithOptimizer, self).__init__(
    ...
    def fit(self, data, sen, event_index=(0, 1)):
        ...
        # call super class
        super(BaseIndependentLogisticPMFWithOptimizer, self).fit(
