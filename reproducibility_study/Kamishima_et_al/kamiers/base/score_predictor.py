...
class BaseIndependentScorePredictorFromSingleBinarySensitive(
        with_metaclass(ABCMeta, BaseScorePredictor)):
    """
    Recommenders to predict preference scores from event data. The 
    prediction is independent from a single binary sensitive feature. 
    ...
    def fit(self, data, sen, event_index=(0, 1)):
        ...
        """
        super(
            BaseIndependentScorePredictorFromSingleBinarySensitive, self).fit(
            data=data, event_index=event_index)
