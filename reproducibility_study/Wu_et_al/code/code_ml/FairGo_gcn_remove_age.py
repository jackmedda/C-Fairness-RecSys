...
class InforMax(nn.Module):
    ...
    def forward(self, adj_pos,user_batch,rating_batch,item_batch):      
        ...
        d_loss_all = _lambda * (d_loss + 1 * d_loss_local)  # +1*d_loss_local #+1*d_loss1_local.cpu().numpy()
...
for epoch in range(100):
    ...
    if val_rmse is not None:
        if r_val < val_rmse:
            val_rmse = r_val
            print("epoch", epoch)
            with open(os.path.join(dataset_base_path, f"predictions_age.pkl"), 'wb') as pk:
