...
item_num=3706#item_size
factor_num=64
batch_size=2048*100
top_k=20
num_negative_test_val=-1##all 

dataset_base_path= '../../movielens_1m_reproduce_data'
saved_model_path='..'  
...
user_rating_set_all, _, _ = np.load(dataset_base_path+'/user_rating_set_all.npy',allow_pickle=True)

training_ratings_dict, train_dict_count = np.load(dataset_base_path+'/training_ratings_dict.npy',allow_pickle=True)
testing_ratings_dict, test_dict_count = np.load(dataset_base_path+'/testing_ratings_dict.npy',allow_pickle=True)
val_ratings_dict, val_dict_count = np.load(dataset_base_path+'/validation_ratings_dict.npy',allow_pickle=True)

training_u_i_set, training_i_u_set = np.load(dataset_base_path+'/training_adj_set.npy',allow_pickle=True)
...
users_emb_gcn = np.load('./new_gcn_embs/user_emb_epoch.npy',allow_pickle=True)
items_emb_gcn = np.load('./new_gcn_embs/item_emb_epoch.npy',allow_pickle=True)

# Gender: 0 = Male, 1 = Female
# Age: 0 = 1-34, 1 = 35-56+
users_features = np.load(dataset_base_path+'/users_features_3num.npy')
...
class InforMax(nn.Module):
    def __init__(self, user_num, item_num, factor_num,users_features,gcn_user_embs,gcn_item_embs):
        ...
        # pdb.set_trace()
        self.users_features = torch.LongTensor(users_features)
        self.gcn_users_embedding0 = torch.FloatTensor(gcn_user_embs)
        self.gcn_items_embedding0 = torch.FloatTensor(gcn_item_embs)
        self.user_num = user_num
        
        self.sigm = nn.Sigmoid()
        self.mse_loss=nn.MSELoss()
        self.model_d1 = filter_layer.DisClfGender(factor_num,2,attribute='gender',use_cross_entropy=True)
        self.model_d2 = filter_layer.DisClfAge(factor_num,2,attribute='age',use_cross_entropy=True)
        ...
        self.real_d = torch.ones(user_num, 1)#Variable(torch.Tensor(user_num).fill_(1.0), requires_grad=False)
        self.fake_d = torch.zeros(user_num, 1)#Variable(torch.Tensor(user_num).fill_(0.0), requires_grad=False)
    ...
    def forward(self, adj_pos,user_batch,rating_batch,item_batch):      
        ...
        _lambda = 0.1  # NOT ORIGINAL CODE
        d_loss_all = _lambda * (d_loss + 0.5 * d_loss_local)  # +1*d_loss_local #+1*d_loss1_local.cpu().numpy()
...
for epoch in range(100):
    ...
    for pair_i in testing_ratings_dict: 
        ...
        pre_test.append(pre_get)
        label_test.append(r_v)

    r_test=rmse(np.array(pre_test),np.array(label_test))
    ...
    for pair_i in val_ratings_dict:
        ...
        pre_get = np.sum(user_e[u_id] * item_e[i_id])
        pre_val.append(pre_get)
        label_val.append(r_v)

    r_val = rmse(np.array(pre_val), np.array(label_val))
    ...
    if val_rmse is not None:
        if r_val < val_rmse:
            val_rmse = r_val
            print("epoch", epoch)
            with open(os.path.join(dataset_base_path, f"predictions_gender.pkl"), 'wb') as pk:
                pickle.dump(pre_test, pk)
    else:
        val_rmse = r_val
