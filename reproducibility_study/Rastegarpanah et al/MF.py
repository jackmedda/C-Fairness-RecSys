...
def reproduce_read_movielens_1M(sensitive_attribute, data_dir='Data/MovieLens-1M'):
    train = pd.read_csv(os.path.join(data_dir, "train.csv"))

    train['user_id'] = train['user_id'].map(lambda x: int(ast.literal_eval(x)))
    train['movie_id'] = train['movie_id'].map(lambda x: int(ast.literal_eval(x)))

    train_ratings = train.pivot(index='user_id', columns='movie_id', values='user_rating')

    test = pd.read_csv(os.path.join(data_dir, "test.csv"))

    test['user_id'] = test['user_id'].map(lambda x: int(ast.literal_eval(x)))
    test['movie_id'] = test['movie_id'].map(lambda x: int(ast.literal_eval(x)))

    test_ratings = test.pivot(index='user_id', columns='movie_id', values='user_rating')
    ...
    train_data['user_rating'] = np.nan
    test_data['user_rating'] = np.nan

    train_data = train_data.pivot(index='user_id', columns='movie_id', values='user_rating')
    test_data = test_data.pivot(index='user_id', columns='movie_id', values='user_rating')

    train_data.update(train_ratings)
    test_data.update(test_ratings)
    ...
    if sensitive_attribute == "Age":
        user_info[sensitive_attribute] = user_info[sensitive_attribute].map(lambda x: "F" if x <= 25 else "M")

    sens_inv = dict(user_info[['UserID', sensitive_attribute]].to_numpy())
    ...
    return train_data, test_data, sens
...
def compute_RMSE(X1,X2,omega, not_omega=False):
    _omega = omega if not_omega else ~omega
    X1 = X1.mask(_omega)
    X2 = X2.mask(_omega)
