...
class LibrecCmd (Cmd):
    ...
    def write_key_value(self, prop_dict):
        ...
        with prop_path.open(mode="w") as fh:
            ...
            for key, value in prop_dict.items():
                ...
                if "movielens" in line:
                    dataset = "movielens_1m"
                elif "lastfm" in line:
                    dataset = "filtered(20)_lastfm_1K"
                fh.write(str(line))
            if dataset is None:
                raise FileNotFoundError("> Reproducibility: cannot find information about dataset in prop values")

            fh.write(f"data.testset.path:{dataset}_test.csv\n")
            #################################
    ...
    def create_proc_spec(self):
        ...
        if java_command is None:
            ...
            return ['java', "-Xmx24g", '-cp', classpath, mainClass, confpath_str, java_command]
